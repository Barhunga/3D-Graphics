...
Mesh::Mesh() : triCount(0), vao(0), vbo(0), ibo(0), 
               Ka(glm::vec3(0)), Kd(glm::vec3(0)), Ks(glm::vec3(0)), specularPower(0), hasNormalMap(false)
{
}
...
void Mesh::initialiseFromFile(const char* filename)
{
    ...
    for (int i = 0; i < numFaces; i++)
    {
        ...
        indices.push_back(mesh->mFaces[i].mIndices[1]); // swap to invert faces
        indices.push_back(mesh->mFaces[i].mIndices[2]); // swap to invert faces
        // generate a second triangle for quads
        if (mesh->mFaces[i].mNumIndices == 4)
        {
            indices.push_back(mesh->mFaces[i].mIndices[0]);                         
            indices.push_back(mesh->mFaces[i].mIndices[2]); // swap to invert faces 
            indices.push_back(mesh->mFaces[i].mIndices[3]); // swap to invert faces 
        }
    }
    ...
    for (int i = 0; i < numV; i++)
    {
        ...
        if (mesh->HasTangentsAndBitangents())
            vertices[i].tangent = glm::vec4(mesh->mTangents[i].x,
                mesh->mTangents[i].y,
...
void Mesh::setMaterial(glm::vec3 ka, glm::vec3 kd, glm::vec3 ks, float ns)
{
    Ka = ka;
    Kd = kd;
    Ks = ks;
